...
(function() {
  this.HumanBody = (function() {
    function HumanBody(head, map, handLeft, handRight, side) {
      this.head = head;
      this.map = map;
      this.handLeft = handLeft;
      this.handRight = handRight;
      this.side = side;
      this.skeleton = new Skeleton(this.head, this.map, this.side);
  ...
  this.Skeleton = (function() {
    function Skeleton(head, map, side) {
      this.head = head;
      this.map = map;
      this.side = side;
      ...
      this.neck = new Point(this.head.x + this.head.width / 2, this.head.y + this.head.height * 0.75);
      this.chestCenter = new Point(this.neck.x, this.neck.y + this.head.height * 1.25);
      this.chestCenter.x = this.centerX(this.map, this.chestCenter.x, this.chestCenter.y);
      this.neck.x = this.chestCenter.x;
      ...
      if (!this.side) {
        this.shoulderRight = new Point(this.chestCenter.x + this.head.width, this.chestCenter.y - this.head.height / 1.5);
        this.shoulderLeft = new Point(this.chestCenter.x - this.head.width, this.chestCenter.y - this.head.height / 1.5);
        this.armRight = new Point(this.shoulderRight.x + this.head.width / 1.5, this.shoulderRight.y + this.head.height * 1.4);
        this.armLeft = new Point(this.shoulderLeft.x - this.head.width / 1.5, this.shoulderLeft.y + this.head.height * 1.4);
        this.handRight = new Point(this.armRight.x + this.head.width / 1.5, this.armRight.y + this.head.height * 1.5);
        this.handLeft = new Point(this.armLeft.x - this.head.width / 1.5, this.armLeft.y + this.head.height * 1.5);
        if (this.fullbody) {
        ...
        this.shoulderRight = new Point(this.chestCenter.x, this.chestCenter.y - this.head.height / 1.5);
        this.shoulderLeft = new Point(this.chestCenter.x, this.chestCenter.y - this.head.height / 1.5);
        this.armRight = new Point(this.shoulderRight.x, this.shoulderRight.y + this.head.height * 1.4);
        this.armLeft = new Point(this.shoulderLeft.x, this.shoulderLeft.y + this.head.height * 1.4);
        ...
        if (this.fullbody) {
          this.legRight = new Point(this.hips.x, this.hips.y);
          this.legLeft = new Point(this.hips.x, this.hips.y);
          this.kneeRight = new Point(this.legRight.x, this.legRight.y + this.head.height * 2);
          this.kneeLeft = new Point(this.legLeft.x, this.legLeft.y + this.head.height * 2);
          this.footRight = new Point(this.kneeRight.x, this.kneeRight.y + this.head.height * 2);
          this.footLeft = new Point(this.kneeLeft.x, this.kneeLeft.y + this.head.height * 2);
    ...
    Skeleton.prototype.centerX = function(map, x, y) {
      var i0, i1, j, value;
      i0 = x;
      i1 = x;
      j = y;
      value = ImageProcessing.getImageValue(map, x, y);
      if (value > 240) {
        while (ImageProcessing.getImageValue(map, i0, j) > 240) {
          i0--;
        }
        while (ImageProcessing.getImageValue(map, i1, j) > 240) {
          i1++;
        }
      }
      return (i0 + i1) / 2;
    ...
    Skeleton.prototype.correlatePoints = function() {
      ...
      gradient = ImageProcessing.dilates(imageDiff, 2);
      gradient = ImageProcessing.blur(gradient, 50);
      gradient = ImageProcessing.forEachPixel(gradient, auxDiff, Math.max);
      gradient = ImageProcessing.blur(gradient, 20);
      auxDiff = ImageProcessing.erodes(imageDiff, 4);
      gradient = ImageProcessing.forEachPixel(gradient, auxDiff, Math.max);
      ...
      chestCenter.x = this.centerX(this.map, chestCenter.x, chestCenter.y);
      this.seekGradient("waist", gradient, waist);
      waist.x = this.centerX(this.map, waist.x, waist.y);
      this.seekGradient("hips", gradient, hips);
      hips.x = this.centerX(this.map, hips.x, hips.y);
      if (!this.side) {
        this.seekGradient("sholderLeft", gradient, shoulderLeft, -0.1, -0.2);
        this.seekGradient("sholderRight", gradient, shoulderRight, 0.1, -0.2);
        ...
        this.goToTheEnds("handLeft", imageDiff, handLeft, -1, 2);
        this.goToTheEnds("handRight", imageDiff, handRight, 1, 2);
        this.seekGradient("handLeft", gradient, handLeft, -0.5, 2);
        this.seekGradient("handRight", gradient, handRight, 0.5, 2);
        ...
        shoulderLeft.x = chestCenter.x;
        shoulderRight.x = chestCenter.x;
        armLeft.x = this.centerX(this.map, armLeft.x, armLeft.y);
        armRight.x = this.centerX(this.map, armRight.x, armRight.y);
        handLeft.x = this.centerX(this.map, handLeft.x, handLeft.y);
        handRight.x = this.centerX(this.map, handRight.x, handRight.y);
        if (this.fullbody) {
          legLeft.x = this.centerX(this.map, legLeft.x, legLeft.y);
          legRight.x = this.centerX(this.map, legRight.x, legRight.y);
          kneeLeft.x = this.centerX(this.map, kneeLeft.x, kneeLeft.y);
          kneeRight.x = this.centerX(this.map, kneeRight.x, kneeRight.y);
          footLeft.x = this.centerX(this.map, footLeft.x, footLeft.y);
          return footRight.x = this.centerX(this.map, footRight.x, footRight.y);
        }
      }
    };
...
(function() {
  ...
  this.Measurements = (function() {
      ...
      return new Measurement("bust", this.currentScale, [
        {
          ...
          y: this.skeleton.chestCenter.y * 1.05
        }, {
          x: this.skeleton.chestCenter.x + this.skeleton.head.width,
          y: this.skeleton.chestCenter.y * 1.05
        }
      ...
      return new Measurement("shoulders", this.currentScale, [
        {
          ...
          y: this.skeleton.shoulderLeft.y - this.skeleton.head.height * 0.3
        }, {
          x: this.skeleton.shoulderRight.x,
          y: this.skeleton.shoulderRight.y - this.skeleton.head.height * 0.3
        }
    ...
    Measurements.prototype.doMeasurements = function(face, map, leftHand, rightHand) {
      this.humanBody = new HumanBody(face, map, leftHand, rightHand, this.side);
    ...
    Measurements.prototype.updateReferenceWithHand = function(handSize) {
      ...
      this.reference.points = [new Point(Math.min(hand.x - auxX, this.humanBody.map.rows * 0.99), Math.min(hand.y - auxY, this.humanBody.map.rows * 0.99)), new Point(Math.min(hand.x + auxX, this.humanBody.map.rows * 0.99), Math.min(hand.y + auxY, this.humanBody.map.rows * 0.99))];
...
(function() {
  this.BackgroundTrackerStep = (function() {
    ...
    BackgroundTrackerStep.prototype.trackBG = function(index) {
      ...
      UI.progress((index + 1) * 100 / this.bgCaptures);
      if (index < this.bgCaptures) {
        ...
        return this.timeoutId = setTimeout("backgroundTracker.trackBG(" + (index + 1) + ")", 100);
      } else {
...
(function() {
  this.BodyMeasurementStep = (function() {
    ...
    BodyMeasurementStep.prototype.startDragging = function(e) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        point = _ref[_i];
        if (point != null ? point.isOver(this.mouseX(e), this.mouseY(e), this.canvasScale) : void 0) {
    ...
    BodyMeasurementStep.prototype.drag = function(e) {
      ...
      if (this.activePoint != null) {
        ...
        return this.measurements.showMeasureHelp();
      } else {
        $(this.canvas).removeClass("moving");
        return this.measurements.hideMeasureHelp();
      }
    };
